...
import { parsePayload, claudeEntries, type HookPayload } from "../src/hookinput.ts";
...
test("edit tools yield exactly one edit entry, whichever path field they use", () => {
  for (const [tool, input, expected] of [
    ...
    const entries = claudeEntries(ev("PostToolUse", tool, input as Record<string, unknown>));
...
test("PostToolUse means the command succeeded; PostToolUseFailure means it failed", () => {
  const ok = claudeEntries(ev("PostToolUse", "Bash", { command: "npm test" }));
  assert.equal(ok[0]!.exitCode, 0);
  const bad = claudeEntries(ev("PostToolUseFailure", "Bash", { command: "npm test" }));
...
test("a payload contradicting its own event records no outcome at all", () => {
  ...
  for (const response of ["exit code 1", "exit 2: boom", "Error: connection refused"]) {
    const e = claudeEntries(ev("PostToolUse", "Bash", { command: "npm test" }, { tool_response: response }));
  ...
  const clean = claudeEntries(ev("PostToolUse", "Bash", { command: "npm test" }, { tool_response: "47 passing" }));
...
test("tools and events we do not model are ignored, never guessed at", () => {
  for (const p of [
    ...
    assert.deepEqual(claudeEntries(p), [], `${p.hook_event_name} ${p.tool_name ?? ""}`);
  }
});
...
test("malformed stdin never throws", () => {
  for (const raw of ["", "   ", "not json", "[]", "null", '{"session_id":', "0"]) {
    assert.doesNotThrow(() => {
      ...
      if (p !== undefined) claudeEntries(p);
    }, `input: ${raw}`);
  }
});
...
test("installation registers both the success and failure channels", () => {
  ...
  const { next } = withProveneHooks({}, CLAUDE);
  ...
  const installed = proveneHooksInstalled(next, CLAUDE);
...
test("installing is idempotent and never disturbs existing hooks", () => {
  ...
  const first = withProveneHooks(existing, CLAUDE);
  const second = withProveneHooks(first.next, CLAUDE);

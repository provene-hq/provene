...
**Pass `--cwd`.** A journal belongs to a session, not to a repository, and a session is free to work in more than one. Without it, a test suite that passed in one project can turn up as verification evidence for a change in another, and `src/a.ts` in two checkouts is the same six characters and a different file.

`emit` drops anything whose recorded directory is outside the repository it is emitting for. Where no directory was recorded, the command is still recorded but **carries no exit code**, and therefore produces no verification run: "it ran" survives, "it ran here and passed" does not. An emitter that omits `--cwd` will find its test evidence silently disappears — which is the intended outcome, because the alternative is a receipt asserting a passing test run on a change the test never saw.
...
| `--cwd` | scoping, not a receipt field | Always. Without it a command yields no verification run |
...
**What has been checked against the runtime, and what has not.** Claims about an integration are worth exactly as much as the thing that established them, so for Gemini CLI 0.57.0 they are separated here.

Verified by running it: hooks written to `~/.gemini/settings.json` are read and fired; `SessionEnd` fires *even when the CLI aborts with a critical error*, so a session that dies badly still emits; `session_id` arrives as a stable UUID, distinct per session; `emit` resolves the repository and completes; and `--quiet` holds on every exit, with the suppressed diagnosis landing in the error journal where `doctor` finds it. The last of those was verified only after a live run proved it false — `--quiet` was honoured on the success path alone, and every ordinary session that ended with nothing to attest was writing prose into a channel documented as JSON-only.

Not yet verified by running it: the `AfterTool` payload — `tool_name`, `tool_input`, `tool_response` — because the sessions available for testing hit an API quota before any tool executed. Those names come from the published reference and, for `is_background`, from the source. They are unit-tested against both. That is weaker than having seen one, and it is listed here rather than glossed, because the two live findings so far were both in the half that had only been read about.
